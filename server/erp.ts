...
export const erpRouter = router({
  ...
  hr: router({
    ...
    reports: protectedProcedure.input(z.object({ branchId: z.number().int().positive().optional(), departmentId: z.number().int().positive().optional() }).optional()).query(async ({ ctx, input }) => { const context = await requireHrPermission(ctx.user.id, "hr.payroll.view"); return getHrOperationalReports(context.organization.id, input); }),
    ...
    createOvertime: protectedProcedure.input(z.object({ employeeId: z.number().int().positive(), occurredAt: z.coerce.date(), hours: z.number().positive(), overtimeType: z.string().trim().min(2).max(64), multiplier: z.number().positive().optional(), reason: z.string().trim().max(2000).optional() })).mutation(async ({ ctx, input }) => { const context = await requireOrganizationOwner(ctx.user.id); return createOvertimeEntry(context.organization.id, ctx.user.id, input); }),
    ...
    assignAllowance: protectedProcedure.input(z.object({ employeeId: z.number().int().positive(), allowanceTypeId: z.number().int().positive(), amount: z.number().nonnegative(), startsAt: z.coerce.date(), endsAt: z.coerce.date().optional() })).mutation(async ({ ctx, input }) => { const context = await requireHrOwner(ctx.user.id); return assignAllowance(context.organization.id, ctx.user.id, input); }),
    createAdjustment: protectedProcedure.input(z.object({ employeeId: z.number().int().positive(), payrollPeriodId: z.number().int().positive().optional(), adjustmentType: z.enum(["deduction", "bonus", "commission", "other"]), amount: z.number().nonnegative(), reason: z.string().trim().min(3).max(2000) })).mutation(async ({ ctx, input }) => { const context = await requireHrOwner(ctx.user.id); return createPayrollAdjustment(context.organization.id, ctx.user.id, input); }),
    createAdvance: protectedProcedure.input(z.object({ employeeId: z.number().int().positive(), occurredAt: z.coerce.date(), amount: z.number().positive(), currencyCode: z.string().trim().length(3), reason: z.string().trim().min(3).max(2000), recoveryMethod: z.enum(["one_payroll", "multiple_payrolls"]), idempotencyKey: z.string().trim().min(8).max(128) })).mutation(async ({ ctx, input }) => { const context = await requireHrOwner(ctx.user.id); return createEmployeeAdvance(context.organization.id, ctx.user.id, input); }),
    createCommissionRule: protectedProcedure.input(z.object({ name: z.string().trim().min(2).max(160), sourceType: z.enum(["sales", "collections", "product", "customer", "target", "route_performance"]), calculationType: z.enum(["fixed", "percentage"]), value: z.number().nonnegative() })).mutation(async ({ ctx, input }) => { const context = await requireHrOwner(ctx.user.id); return createCommissionRule(context.organization.id, ctx.user.id, input); }),
      createCommissionEntry: protectedProcedure.input(z.object({ employeeId: z.number().int().positive(), commissionRuleId: z.number().int().positive().optional(), sourceModule: z.string().trim().min(2).max(64), sourceDocumentType: z.string().trim().min(2).max(64), sourceDocumentId: z.number().int().positive(), occurredAt: z.coerce.date(), amount: z.number().nonnegative(), currencyCode: z.string().trim().length(3) })).mutation(async ({ ctx, input }) => { const context = await requireHrOwner(ctx.user.id); return createCommissionEntry(context.organization.id, ctx.user.id, input); }),
