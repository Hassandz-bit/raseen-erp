...
export async function exportPaidPayrollBankFile(organizationId: number, payrollPeriodId: number, delimiter: "," | ";" = ",") {
  ...
  const exportedRows = valid.map(row => ({ beneficiaryName: row.employeeName, bankAccountReference: row.bankAccountReference!.trim(), amount: Number(row.netPay).toFixed(2), currencyCode: row.currencyCode, reference: `PAYROLL-${payrollPeriodId}-${row.employeeNumber}` }));
  const lines = exportedRows.map(row => [row.beneficiaryName, row.bankAccountReference, row.amount, row.currencyCode, row.reference].map(value => escapeCsv(value, delimiter)).join(delimiter));
  return { filename: `nawa-payroll-bank-${payrollPeriodId}.csv`, content: [header.join(delimiter), ...lines].join("\r\n"), rows: exportedRows, exportedCount: valid.length, excluded };
}
