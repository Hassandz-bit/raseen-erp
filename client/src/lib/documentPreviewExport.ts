...
export function buildDocumentPreviewHtml(data: DocumentPreviewExportData) {
  ...
  return `<!doctype html><html dir="${data.direction}"><head><meta charset="utf-8"><title>${escapeHtml(data.title)}</title><style>body{font-family:${fontFamily},sans-serif;font-size:${fontSize};color:#172033;padding:32px}article{position:relative;isolation:isolate;width:${paper.cssWidth};min-height:${paper.cssHeight};box-sizing:border-box;margin:auto;border:1px solid #d7dce5;border-radius:16px;padding:28px;overflow:hidden}.document-content{position:relative;z-index:1}.header{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;margin-bottom:18px}.logo{display:block;max-width:160px;max-height:64px;object-fit:contain}.legal{max-width:55%;color:#475569;font-size:11px;line-height:1.7;text-align:start}.watermark{position:absolute;inset:18% 12%;width:76%;height:64%;object-fit:contain;opacity:.055;filter:grayscale(1);z-index:0;pointer-events:none}.muted{color:#64748b;font-size:12px}.line{border-top:1px solid #d7dce5;border-bottom:1px solid #d7dce5;margin:24px 0;padding:16px 0}.amount{font-size:24px;font-weight:700}.rows{margin-top:16px;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden}.row{display:flex;justify-content:space-between;gap:16px;padding:10px 12px;border-top:1px solid #e2e8f0}.row:first-child{border-top:0}.signature{border-top:1px solid #94a3b8;margin-top:42px;padding-top:8px;font-size:12px}@media print{body{padding:0}article{border:0}}</style></head><body><article>${watermark}<div class="document-content"><div class="header">${logo}<div class="legal">${legal}</div></div><h1>${escapeHtml(data.title)}</h1><p class="muted">${escapeHtml(data.date)}</p><div class="line"><p>${escapeHtml(data.documentLabel)}</p><p class="amount">${escapeHtml(data.amount)}</p></div>${rows}<p class="muted">${escapeHtml(data.footer ?? "")}</p>${signature}</div></article></body></html>`;
}
