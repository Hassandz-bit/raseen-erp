...
export function buildManufacturingDocument(type: ManufacturingDocumentType, language: ManufacturingDocumentLanguage, input: ManufacturingDocumentInput): DocumentPreviewExportData {
  ...
  const batchRows = [{ label: text.batch, value: input.batchNumber ?? "—" }, { label: text.manufacturingDate, value: formatDate(input.manufacturingDate, language) }, { label: text.expiryDate, value: formatDate(input.expiryDate, language) }, { label: text.quality, value: input.qualityStatus ?? "—" }];
