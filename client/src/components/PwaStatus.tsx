...
export function PwaStatus() {
  ...
  const copy = language === "ar" ? { offline: "أنت الآن دون اتصال. تبقى الواجهة متاحة، لكن البيانات الجديدة تتطلب الاتصال.", update: "يتوفر إصدار جديد من Nawa ERP. حدّث الصفحة لتفعيل الإصدار الجديد.", refresh: "تحديث الصفحة", install: "تثبيت التطبيق", app: "تطبيق الهاتف", title: "ثبّت Nawa ERP كتطبيق", nativeInstall: "تثبيت Nawa ERP الآن", phone: "الهاتف", phoneHelp: "افتح قائمة المتصفح ثم اختر «إضافة إلى الشاشة الرئيسية» أو «تثبيت التطبيق».", desktop: "الكمبيوتر", desktopHelp: "افتح رمز التثبيت بجانب شريط العنوان في Chrome أو Edge ثم اتبع الخطوات.", note: "تبقى الواجهة متاحة بعد التثبيت، بينما تحتاج بيانات المؤسسة الحية إلى اتصال آمن للمزامنة." } : language === "fr" ? { offline: "Vous êtes hors connexion. L’interface reste disponible, mais les nouvelles données exigent une connexion.", update: "Une nouvelle version de Nawa ERP est disponible. Actualisez la page pour l’activer.", refresh: "Actualiser", install: "Installer l’application", app: "Application", title: "Installez Nawa ERP", nativeInstall: "Installer maintenant", phone: "Téléphone", phoneHelp: "Ouvrez le menu du navigateur et choisissez Ajouter à l’écran d’accueil ou Installer l’application.", desktop: "Ordinateur", desktopHelp: "Utilisez l’icône d’installation à côté de la barre d’adresse dans Chrome ou Edge.", note: "L’interface reste disponible après l’installation, mais les données récentes nécessitent une connexion sécurisée." } : { offline: "You are offline. The app shell remains available, but new data requires a connection.", update: "A new Nawa ERP version is available. Refresh the page to activate it.", refresh: "Refresh page", install: "Install app", app: "Mobile app", title: "Install Nawa ERP as an app", nativeInstall: "Install Nawa ERP now", phone: "Phone", phoneHelp: "Open your browser menu and choose Add to Home Screen or Install app.", desktop: "Desktop", desktopHelp: "Use the install icon beside the address bar in Chrome or Edge.", note: "The interface remains available after install, while live organization data needs a secure connection to sync." };
