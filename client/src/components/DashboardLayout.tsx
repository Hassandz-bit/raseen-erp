...
import { Bell, CheckCheck, ChevronDown, ChevronLeft, CircleHelp, Download, Grid2X2, Inbox, Loader2, LogOut, Menu, Plus, Search, Settings2 } from "lucide-react";
...
function DashboardLayoutContent({ children }: { children: ReactNode }) {
  ...
  return <div dir={direction} className="nawa-app-shell">
    <AppHeader
      ...
      navigation={<>{!isPortalLauncher ? <Button variant="ghost" size="icon" onClick={() => setRailOpenOnMobile(true)} aria-label={chrome.tools} title={chrome.tools} className="nawa-mobile-rail-trigger"><Menu className="h-5 w-5" /></Button> : null}<button type="button" onClick={() => navigate("/")} className="nawa-mobile-wordmark sm:hidden" aria-label={chrome.portals} title={chrome.portals}><img src={RASEEN_APP_ICON_URL} alt="" /></button><Button variant="ghost" size="icon" onClick={() => navigate("/")} aria-label={chrome.portals} title={chrome.portals} className="nawa-header-icon hidden sm:inline-grid"><Grid2X2 className="h-5 w-5" /></Button><span className="nawa-workline-divider hidden sm:block" aria-hidden="true" /><span className="nawa-header-breadcrumb truncate">{activeMenuItem?.label[language] ?? chrome.executive}</span></>}
