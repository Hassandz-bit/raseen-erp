...
import { Bell, CheckCheck, ChevronDown, ChevronLeft, CircleHelp, Download, Grid2X2, Inbox, Loader2, LogOut, Menu, MoonStar, Plus, Search, Settings2, SunMedium } from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState, type ReactNode } from "react";
import { toast } from "sonner";
import { useLocation, useSearch } from "wouter";
import { DashboardLayoutSkeleton } from "./DashboardLayoutSkeleton";
import { AppHeader } from "./AppHeader";
import { type NavigationMode, PortalNavigationRail } from "./PortalNavigationRail";
...
function DashboardLayoutContent({ children }: { children: ReactNode }) {
  ...
  return <div dir={direction} className="nawa-app-shell">
    <AppHeader
      brand={<button type="button" onClick={() => navigate("/")} className="nawa-wordmark" aria-label={chrome.portals} title={chrome.portals}><span className="nawa-wordmark-mark"><img src={RASEEN_APP_ICON_URL} alt="" /></span><span className="hidden text-sm font-black tracking-tight sm:inline">RASEEN ERP</span></button>}
      organization={<OrganizationPicker />}
      context={<button type="button" onClick={() => activePortal ? navigate(activePortal.href) : navigate("/executive")} className="nawa-header-context-portal" title={isPortalLauncher ? chrome.portals : activePortal?.name[language] ?? chrome.executive}><PortalIcon className="h-5 w-5 shrink-0" /><span className="truncate">{isPortalLauncher ? chrome.portals : activePortal?.name[language] ?? chrome.executive}</span></button>}
      account={<><span className="nawa-header-usercopy hidden lg:grid"><strong className="truncate">{user?.name || "—"}</strong><small>{chrome.profile}</small></span><DropdownMenu><DropdownMenuTrigger asChild><button type="button" aria-label={chrome.profile} title={chrome.profile} className="rounded-xl p-0.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"><Avatar className="h-9 w-9 border border-primary/20"><AvatarFallback className="bg-primary/10 text-xs font-black text-primary">{user?.name?.charAt(0).toUpperCase() || "R"}</AvatarFallback></Avatar></button></DropdownMenuTrigger><DropdownMenuContent align={direction === "rtl" ? "start" : "end"} className="w-52 rounded-2xl p-1.5"><div className="px-2.5 py-2"><p className="truncate text-sm font-bold">{user?.name || "—"}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{user?.email || "—"}</p></div><DropdownMenuSeparator /><DropdownMenuItem onClick={() => navigate("/settings?section=appearance")} className="cursor-pointer rounded-xl py-2.5"><Settings2 className="me-2 h-4 w-4" />{chrome.preferences}</DropdownMenuItem><DropdownMenuItem onClick={logout} className="cursor-pointer rounded-xl py-2.5 text-destructive focus:text-destructive"><LogOut className="me-2 h-4 w-4" />{t("signOut")}</DropdownMenuItem></DropdownMenuContent></DropdownMenu></>}
      navigation={<>{!isPortalLauncher ? <Button variant="ghost" size="icon" onClick={() => setRailOpenOnMobile(true)} aria-label={chrome.tools} title={chrome.tools} className="nawa-mobile-rail-trigger"><Menu className="h-5 w-5" /></Button> : null}<Button variant="ghost" size="icon" onClick={() => navigate("/")} aria-label={chrome.portals} title={chrome.portals} className="nawa-header-icon hidden sm:inline-grid"><Grid2X2 className="h-5 w-5" /></Button><span className="nawa-workline-divider hidden sm:block" aria-hidden="true" /><span className="nawa-header-breadcrumb truncate">{activeMenuItem?.label[language] ?? chrome.executive}</span></>}
      search={<button type="button" onClick={() => setCommandOpen(true)} className="nawa-command-trigger" aria-label={chrome.portalSearch} title={chrome.portalSearch}><Search className="h-5 w-5 shrink-0" /><span className="truncate">{chrome.portalSearch}</span><kbd className="hidden shrink-0 lg:inline-flex">⌘K</kbd></button>}
      utilities={<>{canQuickCreate && quickActions.length ? <DropdownMenu><DropdownMenuTrigger asChild><Button size="sm" className="nawa-quick-create hidden gap-1.5 sm:inline-flex"><Plus className="h-4 w-4" />{chrome.quickCreate}</Button></DropdownMenuTrigger><DropdownMenuContent align={direction === "rtl" ? "start" : "end"} className="w-56 rounded-2xl p-1.5"><DropdownMenuLabel className="px-2.5 py-2 text-xs text-muted-foreground">{chrome.quickCreateHint}</DropdownMenuLabel><DropdownMenuSeparator />{quickActions.map(action => <DropdownMenuItem key={action.href} onClick={() => navigate(action.href)} className="cursor-pointer rounded-xl py-2.5"><Plus className="me-2 h-4 w-4 text-primary" />{action.label}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu> : null}<Button variant="ghost" size="icon" onClick={toggleTheme} aria-label={theme === "dark" ? chrome.lightMode : chrome.darkMode} title={theme === "dark" ? chrome.lightMode : chrome.darkMode} className={`nawa-header-icon nawa-theme-toggle ${theme === "dark" ? "nawa-theme-toggle-dark" : ""}`}>{theme === "dark" ? <SunMedium className="h-5 w-5" /> : <MoonStar className="h-5 w-5" />}</Button><Button variant="ghost" size="icon" onClick={() => window.dispatchEvent(new Event("nawa-pwa-open-install"))} aria-label={language === "ar" ? "تثبيت التطبيق" : language === "fr" ? "Installer l’application" : "Install app"} title={language === "ar" ? "تثبيت التطبيق" : language === "fr" ? "Installer l’application" : "Install app"} className="nawa-header-icon"><Download className="h-5 w-5" /></Button><NotificationMenu /><Button variant="ghost" size="icon" onClick={() => toast.info(language === "ar" ? "سيضاف مركز المساعدة في تحديث لاحق." : language === "fr" ? "Le centre d’aide sera ajouté dans une prochaine mise à jour." : "The help center will be added in a future update.")} aria-label={language === "ar" ? "المساعدة" : language === "fr" ? "Aide" : "Help"} title={language === "ar" ? "المساعدة" : language === "fr" ? "Aide" : "Help"} className="nawa-header-icon"><CircleHelp className="h-5 w-5" /></Button></>}
    />
    ...
    <CommandDialog open={commandOpen} onOpenChange={open => { setCommandOpen(open); if (!open) setCommandQuery(""); }} title={chrome.commandTitle} description={chrome.commandDescription} className="nawa-command-dialog" showCloseButton={false}>
      <div className="nawa-command-dialog-heading">
        <div className="flex min-w-0 items-center gap-3"><span className="nawa-command-dialog-heading-icon"><Search className="h-4 w-4" /></span><div className="min-w-0"><p className="truncate text-sm font-black text-foreground">{chrome.commandTitle}</p><p className="truncate text-xs text-muted-foreground">{chrome.commandDescription}</p></div></div>
        <kbd className="nawa-command-escape-key">ESC</kbd>
      </div>
      <CommandInput value={commandQuery} onValueChange={setCommandQuery} placeholder={chrome.portalSearch} />
      <div className="nawa-command-list-shell">
      <CommandList className="nawa-command-result-list">
        ...
        {commandEntitySearch.data?.products.length ? <CommandGroup className="nawa-command-section" heading={language === "ar" ? "المنتجات" : language === "fr" ? "Produits" : "Products"}>{commandEntitySearch.data.products.map(result => <CommandItem className="nawa-command-result" key={`product-${result.id}`} value={`product ${result.label} ${result.detail}`} onSelect={() => { navigate("/commerce/products"); setCommandOpen(false); }}><Search /><span className="min-w-0 flex-1 truncate">{result.label}</span><CommandShortcut className="nawa-command-result-meta">{result.detail}</CommandShortcut></CommandItem>)}</CommandGroup> : null}
        {commandEntitySearch.data?.customers.length ? <CommandGroup className="nawa-command-section" heading={language === "ar" ? "العملاء" : language === "fr" ? "Clients" : "Customers"}>{commandEntitySearch.data.customers.map(result => <CommandItem className="nawa-command-result" key={`customer-${result.id}`} value={`customer ${result.label} ${result.detail}`} onSelect={() => { navigate("/commerce/sales"); setCommandOpen(false); }}><Search /><span className="min-w-0 flex-1 truncate">{result.label}</span><CommandShortcut className="nawa-command-result-meta">{result.detail}</CommandShortcut></CommandItem>)}</CommandGroup> : null}
        {commandEntitySearch.data?.invoices.length ? <CommandGroup className="nawa-command-section" heading={language === "ar" ? "الفواتير" : language === "fr" ? "Factures" : "Invoices"}>{commandEntitySearch.data.invoices.map(result => <CommandItem className="nawa-command-result" key={`invoice-${result.id}`} value={`invoice ${result.label} ${result.detail}`} onSelect={() => { navigate(`/commerce/sales/${result.id}`); setCommandOpen(false); }}><Search /><span className="min-w-0 flex-1 truncate">{result.label}</span><CommandShortcut className="nawa-command-result-meta">{result.detail}</CommandShortcut></CommandItem>)}</CommandGroup> : null}
        {activePortal ? <CommandGroup className="nawa-command-section" heading={`${activePortal.name[language]} · ${chrome.pages}`}>{(commandQuery.trim() ? localItems : localItems.slice(0, 6)).map(item => { const ItemIcon = getPortalNavigationIcon(item.id, activePortal.icon); return <CommandItem className="nawa-command-result" key={`active-${item.id}`} value={`${item.label[language]} ${item.group[language]}`} onSelect={() => { navigate(item.href); setCommandOpen(false); }}><ItemIcon /><span className="min-w-0 flex-1 truncate">{item.label[language]}</span><CommandShortcut className="nawa-command-result-meta">{item.group[language]}</CommandShortcut></CommandItem>; })}</CommandGroup> : null}
        {!activePortal || commandQuery.trim() ? <CommandGroup className="nawa-command-section" heading={chrome.portals}>{allowedPortals.map(portal => { const Icon = portal.icon; return <CommandItem className="nawa-command-result" key={portal.id} value={`${portal.name[language]} ${portal.description[language]}`} onSelect={() => { navigate(portal.href); setCommandOpen(false); }}><Icon /><span className="min-w-0 flex-1 truncate">{portal.name[language]}</span><CommandShortcut className="nawa-command-result-meta">{chrome.discovery}</CommandShortcut></CommandItem>; })}</CommandGroup> : null}
      </CommandList>
      </div>
    </CommandDialog>
