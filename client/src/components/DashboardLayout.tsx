...
import { Bell, Bot, CheckCheck, ChevronDown, ChevronLeft, Download, Grid2X2, Home, Inbox, Loader2, LogOut, Menu, Pin, PinOff, Plus, Printer, Search, Settings2, X } from "lucide-react";
  ...
  return <div dir={direction} className="nawa-app-shell">
    <header className="nawa-global-header nawa-command-bar">
      ...
      <div className="nawa-command-actions">
        ...
        <Button variant="ghost" size="icon" onClick={() => window.print()} aria-label={chrome.print} className="nawa-header-icon"><Printer className="h-5 w-5" /></Button>
    ...
    <div className={`nawa-shell-body ${isPortalLauncher ? "nawa-shell-body-launcher" : ""}`}>
      ...
      <div className="nawa-workspace"><div className="nawa-workspace-inner"><div className="nawa-print-only"><strong>RASEEN ERP</strong><span>{activePortal?.name[language] ?? chrome.executive}{activeMenuItem ? ` · ${activeMenuItem.label[language]}` : ""}</span><span>{new Intl.DateTimeFormat(language === "ar" ? "ar" : language === "fr" ? "fr-FR" : "en-US", { dateStyle: "medium", timeStyle: "short" }).format(new Date())}</span></div>{!isPortalLauncher && activeMenuItem ? <div className="nawa-page-context"><span className="text-primary">{activePortal?.name[language]}</span><ChevronLeft className="h-3.5 w-3.5 text-muted-foreground" /><span>{activeMenuItem.label[language]}</span></div> : null}{children}</div></div>
    </div>
  </div>;
}
