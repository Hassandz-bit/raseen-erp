...
import { modulesPageCopy } from "@/i18n/translations";
...
export default function ModulesPage() {
  ...
  const copy = modulesPageCopy[language];
  ...
  return <DashboardLayout><main className="mx-auto max-w-7xl space-y-7" dir={language === "ar" ? "rtl" : "ltr"}><section className="surface relative overflow-hidden rounded-3xl border p-7 md:p-9"><div className="absolute -left-12 -top-16 h-60 w-60 rounded-full bg-primary/10 blur-3xl" /><div className="relative"><div className="flex items-center gap-2 text-primary"><Boxes className="h-4 w-4" /><p className="text-sm font-semibold">{t("moduleHub")}</p></div><h1 className="mt-3 text-3xl font-bold text-white">{copy.heroTitle}</h1><p className="mt-3 max-w-3xl text-sm leading-8 text-muted-foreground">{copy.heroDescription}</p></div></section><section className="grid gap-5 md:grid-cols-2 2xl:grid-cols-3">{modules.map(module => { const active = module.entitlement.some(key => activeKeys.has(key)); const Icon = module.icon; const portalHref = module.key === "inventory" ? "/commerce" : module.key === "distribution" ? "/distribution" : module.key === "manufacturing" ? "/manufacturing" : module.key === "finance" ? "/finance" : "/hr"; return <article key={module.key} className="surface group relative overflow-hidden rounded-3xl border p-6"><div className={`absolute inset-x-0 top-0 h-28 bg-gradient-to-b ${module.accent}`} /><div className="relative"><div className="flex items-start justify-between"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-white/[.07] text-primary ring-1 ring-white/10"><Icon className="h-6 w-6" /></div>{active ? <span className="flex items-center gap-1 rounded-full bg-emerald-400/10 px-2.5 py-1 text-[11px] font-semibold text-emerald-300"><ShieldCheck className="h-3.5 w-3.5" />{t("active")}</span> : <span className="flex items-center gap-1 rounded-full bg-white/[.06] px-2.5 py-1 text-[11px] font-semibold text-slate-300"><LockKeyhole className="h-3.5 w-3.5" />{t("locked")}</span>}</div><p className="mt-10 text-xs font-semibold text-primary">{t("premiumModule")}</p><h2 className="mt-2 text-xl font-bold text-white">{copy.titles[module.title]}</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">{active ? copy.activeDescription : t("noAccess")}</p><div className="mt-6">{active ? <Button onClick={() => setLocation(portalHref)} className="rounded-xl bg-primary text-primary-foreground">{t("explore")}</Button> : <Button variant="outline" onClick={() => toast.info(copy.upgradeRequested)} className="gap-2 rounded-xl border-white/10 bg-white/[.03] text-slate-200"><Sparkles className="h-4 w-4" />{t("explore")}</Button>}</div></div></article>})}</section></main></DashboardLayout>;
}
