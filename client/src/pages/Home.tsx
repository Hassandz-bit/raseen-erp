...
function ModuleView({ section, onBack }: { section: Exclude<SectionKey, "dashboard">; onBack: () => void }) {
  ...
  const rows = section === "inventory" ? products.map(product => ({ ...product, stock: formatNumber(product.stock), status: t(product.statusKey as never) })) : section === "sales" ? invoices.map(i => ({ name: i.customer, sku: i.no, stock: i.value, status: t(i.statusKey as never), tone: i.tone })) : products.map((p, index) => ({ ...p, name: ui.fallbackNames[index] ?? p.name, sku: ui.fallbackPeriods[index] ?? p.sku, stock: formatNumber(p.stock), status: t(p.statusKey as never) }));
