    ...
    <div className="enter space-y-6">
      <section className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" onClick={() => onOpenModule("reports")} className="gap-2 rounded-xl border-white/10 bg-white/[.03] text-slate-200 hover:bg-white/[.07]"><FileBarChart2 className="h-4 w-4" />{t("reports")}</Button>
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,0.85fr)]">
        ...
        <article className="surface rounded-3xl border p-5">
          <div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-white">{t("decisionAlerts")}</p><p className="mt-1 text-xs text-muted-foreground">{t("todayPriority")}</p></div><Bell className="h-4 w-4 text-primary" /></div>
          <div className="mt-5 space-y-3">
            {[{ icon: Package, titleKey: "criticalStockAlert", detailKey: "criticalStockDetail", tone: "red" }, { icon: ReceiptText, titleKey: "dueInvoiceAlert", detailKey: "dueInvoiceDetail", tone: "amber" }, { icon: Truck, titleKey: "purchaseReceiptAlert", detailKey: "purchaseReceiptDetail", tone: "blue" }].map(item => <button key={item.titleKey} onClick={() => toast.info(t(item.titleKey as never))} className="flex w-full items-center gap-3 rounded-2xl border border-white/[.06] bg-white/[.025] p-3 text-right transition-colors hover:bg-white/[.06]"><div className={cn("grid h-9 w-9 shrink-0 place-items-center rounded-xl", item.tone === "red" ? "bg-rose-400/10 text-rose-300" : item.tone === "amber" ? "bg-amber-300/10 text-amber-200" : "bg-sky-300/10 text-sky-200")}><item.icon className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="text-xs font-semibold text-slate-100">{t(item.titleKey as never)}</p><p className="mt-1 truncate text-[11px] text-muted-foreground">{t(item.detailKey as never)}</p></div><ChevronLeft className="h-4 w-4 text-muted-foreground" /></button>)}
          </div>
        </article>
      </section>
    </div>
...
export function ModuleView({ section, onBack }: { section: Exclude<SectionKey, "dashboard">; onBack: () => void }) {
  ...
  const rows = section === "inventory" ? products.map((product, index) => ({ ...product, name: ui.sampleProducts[index] ?? product.name, stock: formatNumber(product.stock), status: t(product.statusKey as never) })) : section === "sales" ? invoices.map((invoice, index) => ({ name: ui.sampleCustomers[index] ?? invoice.customer, sku: invoice.no, stock: formatCurrency(invoice.value), status: t(invoice.statusKey as never), tone: invoice.tone })) : products.map((p, index) => ({ ...p, name: ui.fallbackNames[index] ?? p.name, sku: ui.fallbackPeriods[index] ?? p.sku, stock: formatNumber(p.stock), status: t(p.statusKey as never) }));
  return <div className="enter space-y-6"><section className="surface relative overflow-hidden rounded-3xl border p-6 md:p-8"><div className="absolute -left-14 -top-14 h-48 w-48 rounded-full bg-primary/10 blur-3xl" /><div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-end"><div className="flex items-start gap-4"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-primary/10 text-primary ring-1 ring-primary/20"><Icon className="h-6 w-6" /></div><div><p className="text-sm text-primary">{info.eyebrow}</p><h1 className="mt-1 text-2xl font-bold text-white">{info.title}</h1><p className="mt-2 max-w-2xl text-sm leading-7 text-muted-foreground">{info.detail}</p></div></div><div className="flex items-center gap-2"><Button variant="outline" onClick={onBack} className="rounded-xl border-white/10 bg-white/[.03] text-slate-200">{ui.backToDashboard}</Button><Button onClick={() => toast.success(info.action)} className="gap-2 rounded-xl bg-primary text-primary-foreground"><Plus className="h-4 w-4" />{info.action}</Button></div></div></section><section className="grid gap-5 lg:grid-cols-[minmax(0,1.4fr)_330px]"><article className="surface overflow-hidden rounded-3xl border"><div className="flex items-center justify-between border-b border-white/8 px-5 py-5"><div><p className="text-sm font-semibold text-white">{ui.recentRecords}</p><p className="mt-1 text-xs text-muted-foreground">{ui.previewFor(info.eyebrow)}</p></div><Button variant="outline" onClick={() => toast.info(ui.filterApplied)} className="h-9 rounded-xl border-white/10 bg-white/[.03] text-xs text-slate-300">{ui.filter}</Button></div><div className="thin-scrollbar overflow-x-auto"><table className="w-full min-w-[600px] text-right"><thead className="bg-white/[.025] text-[11px] text-muted-foreground"><tr><th className="px-5 py-3 font-medium">{ui.item}</th><th className="px-5 py-3 font-medium">{ui.reference}</th><th className="px-5 py-3 font-medium">{ui.valueOrQuantity}</th><th className="px-5 py-3 font-medium">{ui.status}</th></tr></thead><tbody>{rows.map((row, index) => <tr key={`${row.name}-${index}`} className="border-t border-white/[.055] hover:bg-white/[.025]"><td className="px-5 py-4 text-sm text-white">{row.name}</td><td className="latin px-5 py-4 text-xs text-muted-foreground">{row.sku}</td><td className="px-5 py-4 text-sm text-slate-300">{row.stock}</td><td className="px-5 py-4"><StatusPill tone={row.tone}>{row.status}</StatusPill></td></tr>)}</tbody></table></div></article><article className="surface rounded-3xl border p-5"><p className="text-sm font-semibold text-white">{ui.quickSummary}</p><div className="mt-5 rounded-2xl border border-primary/15 bg-primary/[.07] p-4"><p className="text-xs text-primary">{ui.moduleMetric}</p><p className="mt-2 text-lg font-bold text-white">{info.stat}</p><p className="mt-2 text-xs leading-6 text-muted-foreground">{ui.emptyMetrics}</p></div><button onClick={() => toast.info(ui.customizationAvailable)} className="mt-5 flex w-full items-center justify-between rounded-2xl bg-white/[.04] p-4 text-sm text-slate-200 hover:bg-white/[.07]"><span>{ui.customizeModule}</span><Settings2 className="h-4 w-4 text-primary" /></button></article></section></div>;
}
