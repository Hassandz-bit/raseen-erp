...
export function ModuleView({ section, onBack }: { section: Exclude<SectionKey, "dashboard">; onBack: () => void }) {
  ...
  const rows = section === "inventory" ? products.map((product, index) => ({ ...product, name: ui.sampleProducts[index] ?? product.name, stock: formatNumber(product.stock), status: t(product.statusKey as never) })) : section === "sales" ? invoices.map((invoice, index) => ({ name: ui.sampleCustomers[index] ?? invoice.customer, sku: invoice.no, stock: invoice.value, status: t(invoice.statusKey as never), tone: invoice.tone })) : products.map((p, index) => ({ ...p, name: ui.fallbackNames[index] ?? p.name, sku: ui.fallbackPeriods[index] ?? p.sku, stock: formatNumber(p.stock), status: t(p.statusKey as never) }));
