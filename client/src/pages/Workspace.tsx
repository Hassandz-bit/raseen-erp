...
function InsightsPanel({ modules }: { modules: { key: string; status: string }[] }) {
  ...
  if (commerce.isError || notifications.isError) return <section className="grid gap-5 xl:grid-cols-4" dir={direction}><article className="surface col-span-full rounded-3xl border"><WorkspaceState label={t("error")} tone="error" /></article></section>;
...
function WorkspaceContent({ organizationName, activeModules, modules }: { organizationName: string; activeModules: number; modules: { key: string; status: string }[] }) {
  ...
  return (
    <section className="mx-auto max-w-6xl space-y-6" dir={direction}>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_290px]"><AIChatBox messages={messages} onSendMessage={handleSendMessage} isLoading={askAssistant.isPending} height="590px" className="surface overflow-hidden rounded-3xl border" emptyStateMessage={assistantCopy.chatEmpty} placeholder={assistantCopy.chatPlaceholder} suggestedPrompts={[assistantCopy.promptMetrics, assistantCopy.promptCommerce, assistantCopy.promptRecommendation]} /><aside className="space-y-4"><div className="surface rounded-3xl border p-5"><div className="flex items-center gap-2 text-primary"><Sparkles className="h-4 w-4" /><p className="text-sm font-semibold">{t("assistantScope")}</p></div><p className="mt-3 text-xs leading-7 text-muted-foreground">{assistantCopy.scopeDescription}</p></div><div className="rounded-3xl border border-primary/15 bg-primary/[.06] p-5"><p className="text-xs font-semibold text-primary">{assistantCopy.practicalSuggestion}</p><p className="mt-2 text-sm font-bold leading-7 text-white">{assistantCopy.practicalSuggestionDetail}</p><p className="mt-2 text-xs leading-6 text-muted-foreground">{assistantCopy.practicalSuggestionNote}</p></div><div className="surface rounded-3xl border p-5"><div className="flex items-center gap-2 text-primary"><BellRing className="h-4 w-4" /><p className="text-sm font-semibold">{t("evaluateAlerts")}</p></div><p className="mt-2 text-xs leading-6 text-muted-foreground">{assistantCopy.evaluateDescription}</p><Button onClick={() => evaluateAlerts.mutate()} disabled={evaluateAlerts.isPending} className="mt-4 h-10 w-full rounded-xl bg-primary text-primary-foreground">{evaluateAlerts.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : t("checkNow")}</Button>{alertReasons.length > 0 && <ul className="mt-3 space-y-2 text-[11px] leading-6 text-amber-200">{alertReasons.map(reason => <li key={reason} className="rounded-xl bg-amber-300/10 px-3 py-2">{reason}</li>)}</ul>}</div></aside></div>
      <OperationsPanel />
