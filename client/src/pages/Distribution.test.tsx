...
vi.mock("@/lib/trpc", () => ({ trpc: { erp: { distribution: { controlCenter: { useQuery: () => ({ data: { routesToday: 4, activeRoutes: 2, vehiclesLoaded: 1, pendingDeliveries: 3, collections: 1200, returns: 5, capacityUtilization: 0.65 }, isLoading: false, isError: false, refetch: vi.fn() }) }, customers: { useQuery: () => ({ data: [{ id: 7, code: "CUS-007", name: "عميل تجريبي" }], isLoading: false, isError: false, refetch: vi.fn() }) }, routes: { list: { useQuery: () => ({ data: [], isLoading: false, isError: false, refetch: vi.fn() }) }, create: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, vehicles: { list: { useQuery: () => ({ data: [], isLoading: false, isError: false, refetch: vi.fn() }) }, documentAlerts: { useQuery: () => ({ data: [], isLoading: false, isError: false, refetch: vi.fn() }) }, create: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, territories: { list: { useQuery: () => ({ data: [], isLoading: false, isError: false, refetch: vi.fn() }) }, create: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, settings: { get: { useQuery: () => ({ data: { overloadPolicy: "warning" }, isLoading: false, isError: false, refetch: vi.fn() }) } } } } } }));
...
describe("Distribution UI", () => {
  it("يعرض مؤشرات المركز وحالات الفراغ ويصدر جدول Excel", () => {
    ...
    fireEvent.click(screen.getByText("جولة جديدة"));
    expect(screen.getByText("محطة العميل")).toBeTruthy();
