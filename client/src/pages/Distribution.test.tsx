...
vi.mock("@/lib/trpc", () => ({ trpc: { erp: { distribution: { controlCenter: { useQuery: () => ({ data: { routesToday: 4, activeRoutes: 2, vehiclesLoaded: 1, pendingDeliveries: 3, collections: 1200, returns: 5, capacityUtilization: 0.65 }, isLoading: false, isError: false, refetch: vi.fn() }) }, routes: { list: { useQuery: () => ({ data: [], isLoading: false, isError: false, refetch: vi.fn() }) }, create: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, vehicles: { list: { useQuery: () => ({ data: [], isLoading: false, isError: false, refetch: vi.fn() }) }, create: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, territories: { list: { useQuery: () => ({ data: [], isLoading: false, isError: false, refetch: vi.fn() }) }, create: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, settings: { get: { useQuery: () => ({ data: { overloadPolicy: "warning" }, isLoading: false, isError: false, refetch: vi.fn() }) } } } } } }));
