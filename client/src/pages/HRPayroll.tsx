...
import { Tabs as RadixTabs, TabsContent } from "@/components/ui/tabs";
...
export default function HRPayroll() {
  ...
  return <DashboardLayout><main dir={direction} className="mx-auto w-full max-w-[1500px] space-y-6 p-4 md:p-7">
    <section className="rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/15 via-card to-card p-6 shadow-2xl"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="mb-2 flex items-center gap-2 text-primary"><UsersRound className="h-5 w-5" /><span className="text-sm font-semibold">RASEEN ERP</span></div><h1 className="text-3xl font-bold tracking-tight">{text.title}</h1><p className="mt-2 max-w-3xl text-sm text-muted-foreground">{text.subtitle}</p></div><div className="flex max-w-full flex-wrap items-center gap-2"><Button variant="outline" onClick={refresh} disabled={isLoading}><RefreshCcw className={`me-2 h-4 w-4 ${isLoading ? "animate-spin" : ""}`} />{text.retry}</Button><Badge className="max-w-full whitespace-normal text-center leading-5 bg-primary/15 text-primary hover:bg-primary/15">{text.confidential}</Badge></div></div></section>
    ...
    <Tabs defaultValue="overview" onValueChange={tab => setLocation(tab === "overview" ? "/hr" : `/hr/${tab}`)} className="space-y-5">
