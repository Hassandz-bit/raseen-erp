...
vi.mock("@/lib/trpc", () => ({ trpc: { erp: { distribution: { driver: { myRoutes: { useQuery: () => ({ data: [{ id: 33, routeNumber: "RTE-33", routeDate: new Date(), status: "started", vehicleId: 8, vehicleCode: "V-8", stops: [{ id: 17, customerId: 72, sequence: 1, deliveryStatus: "arrived", customerName: "عميل الاختبار", customerAddress: "الجزائر", customerLatitude: "36.75", customerLongitude: "3.05" }] }], isLoading: false, isError: false, refetch: vi.fn() }) }, inventory: { useQuery: () => ({ data: [{ batchId: 5, productId: 9, productName: "منتج الاختبار", sku: "SKU-9", unit: "PCS", lotNumber: "LOT-1", expiryDate: null, loadedQuantity: "5", remainingQuantity: "5" }], refetch: vi.fn() }) }, transition: { useMutation: () => ({ mutate: transitionMutate, isPending: false }) }, submitProof: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, vanSale: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, completeStop: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, deliveries: { record: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, collections: { record: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, returns: { record: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, expenses: { create: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, closings: { submit: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, tracking: { location: { useMutation: () => ({ mutate: vi.fn(), mutateAsync: vi.fn() }) }, geofence: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } } } } } }));
