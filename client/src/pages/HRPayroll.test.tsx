...
vi.mock("@/lib/trpc", () => ({ trpc: { erp: { hr: { dashboard: { useQuery: () => ({ data: { totalEmployees: 4, activeEmployees: 3, presentToday: 2, absentToday: 1, onLeave: 1 } }) }, directory: { useQuery: () => ({ data: [] }) }, operations: { useQuery: () => ({ data: { leaves: [], overtime: [] } }) }, payrollDashboard: { useQuery: () => ({ data: { totals: { gross: 0, net: 0, outstandingAdvances: 0 }, periods: [] } }) } } } } }));
