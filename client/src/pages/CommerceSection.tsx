...
import { Boxes, ChevronLeft, Download, Eye, Loader2, PackageSearch, ReceiptText, RefreshCw, ShoppingCart, Warehouse } from "lucide-react";
...
export default function CommerceSectionPage() {
  ...
  const table = () => {
    ...
    if (section === "products") return <table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b bg-muted/30 text-start text-xs text-muted-foreground"><th className="p-4">{labels.name}</th><th className="p-4">SKU</th><th className="p-4">{labels.price}</th><th className="p-4">{labels.status}</th><th className="p-4">{labels.details}</th></tr></thead><tbody>{displayedRows.map((product: any) => <tr key={product.id} className="border-b border-border/60 hover:bg-muted/20"><td className="p-4 font-medium">{product.name}</td><td className="p-4 font-mono text-xs">{product.sku}</td><td className="p-4">{formatCurrency(Number(product.salePrice))}</td><td className="p-4">{statusBadge(product.status, t(product.status))}</td><td className="p-4">{detailButton(product.id)}</td></tr>)}</tbody></table>;
    if (section === "warehouses") return <table className="w-full min-w-[700px] text-sm"><thead><tr className="border-b bg-muted/30 text-start text-xs text-muted-foreground"><th className="p-4">{labels.code}</th><th className="p-4">{labels.name}</th><th className="p-4">{labels.status}</th><th className="p-4">{labels.details}</th></tr></thead><tbody>{displayedRows.map((warehouse: any) => <tr key={warehouse.id} className="border-b border-border/60 hover:bg-muted/20"><td className="p-4 font-mono text-xs">{warehouse.code}</td><td className="p-4 font-medium">{warehouse.name}</td><td className="p-4">{statusBadge(warehouse.status ?? "active", t(warehouse.status ?? "active"))}</td><td className="p-4">{detailButton(warehouse.id)}</td></tr>)}</tbody></table>;
    if (section === "batches") return <table className="w-full min-w-[900px] text-sm"><thead><tr className="border-b bg-muted/30 text-start text-xs text-muted-foreground"><th className="p-4"><Checkbox aria-label={labels.selected} checked={displayedRows.length > 0 && displayedRows.every((batch: any) => selectedBatchIds.includes(batch.id))} onCheckedChange={checked => setSelectedBatchIds(checked ? displayedRows.map((batch: any) => batch.id) : [])} /></th><th className="p-4">{labels.code}</th><th className="p-4">{labels.quantity}</th><th className="p-4">{labels.expiry}</th><th className="p-4">{labels.status}</th><th className="p-4">{labels.details}</th></tr></thead><tbody>{displayedRows.map((batch: any) => <tr key={batch.id} className="border-b border-border/60 hover:bg-muted/20"><td className="p-4"><Checkbox aria-label={batch.lotNumber} checked={selectedBatchIds.includes(batch.id)} onCheckedChange={checked => toggleBatch(batch.id, checked === true)} /></td><td className="p-4 font-mono text-xs">{batch.lotNumber}</td><td className="p-4">{formatNumber(Number(batch.currentQuantity))}</td><td className="p-4">{batch.expiryDate ? formatDate(batch.expiryDate) : "—"}</td><td className="p-4">{statusBadge(batch.status, t(batch.status))}</td><td className="p-4">{detailButton(batch.id)}</td></tr>)}</tbody></table>;
    if (section === "sales") return <table className="w-full min-w-[1120px] text-sm"><thead><tr className="border-b bg-muted/30 text-start text-xs text-muted-foreground"><th className="p-4">{labels.document}</th><th className="p-4">{labels.netAmount}</th><th className="p-4">{labels.taxAmount}</th><th className="p-4">{labels.total}</th><th className="p-4">{labels.priceMode}</th><th className="p-4">{labels.status}</th><th className="p-4">{labels.details}</th></tr></thead><tbody>{(displayedRows as any[]).map(invoice => <tr key={invoice.id} className="border-b border-border/60 hover:bg-muted/20"><td className="p-4 font-mono text-xs">{invoice.invoiceNumber}</td><td className="p-4">{formatCurrency(Number(invoice.netAmount))}</td><td className="p-4">{formatCurrency(Number(invoice.taxAmount))}</td><td className="p-4 font-semibold">{formatCurrency(Number(invoice.grandTotal))}</td><td className="p-4">{invoice.taxMode === "inclusive" ? labels.inclusive : labels.exclusive}</td><td className="p-4">{statusBadge(invoice.status, t(invoice.status))}</td><td className="p-4">{detailButton(invoice.id)}</td></tr>)}</tbody></table>;
    const documents = displayedRows as any[];
    return <table className="w-full min-w-[880px] text-sm"><thead><tr className="border-b bg-muted/30 text-start text-xs text-muted-foreground"><th className="p-4">{labels.document}</th><th className="p-4">{labels.total}</th><th className="p-4">{labels.status}</th><th className="p-4">{labels.details}</th></tr></thead><tbody>{documents.map(document => <tr key={document.id} className="border-b border-border/60 hover:bg-muted/20"><td className="p-4 font-mono text-xs">{document.invoiceNumber ?? document.orderNumber}</td><td className="p-4">{formatCurrency(Number(document.grandTotal))}</td><td className="p-4">{statusBadge(document.status, t(document.status))}</td><td className="p-4">{detailButton(document.id)}</td></tr>)}</tbody></table>;
  };
