...
vi.mock("@/lib/trpc", () => ({ trpc: { useUtils: () => ({ erp: { b2b: { management: { orders: { invalidate: vi.fn() }, accesses: { invalidate: vi.fn() } } } } }), erp: { b2b: { management: { orders: { useQuery: () => ({ data: [{ id: 1, orderNumber: "B2B-1", retailerName: "محل النواة", status: "new", accessStatus: "active", totalAmount: "120", currencyCode: "DZD", items: [{ id: 1, productId: 4, unit: "PCS", quantity: "2", unitPrice: "60" }] }], isLoading: false }) }, accesses: { useQuery: () => ({ data: [], isLoading: false }) }, accessStatus: { useMutation: () => ({ mutate: vi.fn() }) } }, review: { useMutation: () => ({ mutate: vi.fn() }) } } } } }));
