...
import { downloadSalesInvoicePdf } from "@/lib/salesInvoicePdfExport";
import { trpc } from "@/lib/trpc";
import { ArrowRight, Download, FileText, Loader2, RefreshCw } from "lucide-react";
...
export default function CommerceRecordDetail() {
  ...
  const copy = language === "ar" ? { back: "العودة إلى الجدول", details: "تفاصيل السجل", missing: "لم يُعثر على هذا السجل أو لا تملك صلاحية قراءته.", reload: "إعادة المحاولة", identifier: "معرّف السجل", downloadPdf: "تنزيل PDF رسمي", pdfError: "تعذر إنشاء ملف PDF للفاتورة." } : language === "fr" ? { back: "Retour au tableau", details: "Détails de l’enregistrement", missing: "Enregistrement introuvable ou accès non autorisé.", reload: "Réessayer", identifier: "Identifiant", downloadPdf: "Télécharger le PDF officiel", pdfError: "Impossible de créer le PDF de la facture." } : { back: "Back to table", details: "Record details", missing: "Record not found or you do not have access.", reload: "Retry", identifier: "Record ID", downloadPdf: "Download official PDF", pdfError: "Unable to create the invoice PDF." };
